...
bool Possible(int arr[],int c,int n,int mid)
{
    int cow=1;
    int lastPos = arr[0];
    for(int i=1;i<n;i++)
...
int FindMinLargestDistance(int arr[], int c, int n)
{
    int s = 0;
    int e = arr[n-1] - arr[0];
    int ans = 0;
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
        cin >> arr[i];
    sort(arr, arr + n);
    cout << FindMinLargestDistance(arr, c, n) << endl;
}